...
void setup() {
  ...
  int weightsLayerLengths[] = {0, 64};
  int biasLayerLenghts[] = {0, 8};
  ...
  for (int currentLayer = 1; currentLayer < numberOfLayers; currentLayer++)
  {
    ...
    int numberOfPreviousUnits = unitsInLayer[currentLayer - 1];
    int numberOfCurrentUnits = unitsInLayer[currentLayer];
    ...
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        ...
        out[n * i + j] = biases[biasLayerLenghts[currentLayer-1]+i];
        for (int k = 0; k < p; k++) {
          out[n * i + j] = out[n * i + j] + weights[(p*i+k)+weightsLayerLengths[currentLayer-1]] * input[n * k + j];
          Serial.print((p*i+k)+weightsLayerLengths[currentLayer-1]);
          Serial.print(",");
        }
...
int findmaxvalue() {
  ...
  return 8;
  // go through array and return max value
}
void loop() {
  /*
    Number of units in layer as array: [4,1]
    All biases in one matrix:
    Biases   Layer 0  1
                   x  x
                   x  0
                   x  0
                   x  0
    All weight_matrizes in weight_matrix_array

    1. Check if input vector has same dimension (row or column) as weight matrix[0]
    2. Create neuron_input_vector with length of array[0]
    3. Copy input values to neuron_input_vector
    4. Multiply weight_matrix_array[i=0] with neuron_input_vector
      4a. Multiply entity
      4b. Add bias
      4b. Apply activationfunction
    5. Store result in neuron_output_vector with length of [i+1=1]
    6. Store neuron_output_vector in neuron_input_vector
    7. Release neuron_output vector // Remove from memory
    8. Go to 4.
